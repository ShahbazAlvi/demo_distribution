...
class _RecoveryListScreenState extends State<RecoveryListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<RecoveryVoucher> records =
        provider.recoveryData?.data.vouchers ?? [];
                    ...
                    child: InkWell(
                      ...
                      onTap: () async {
                        String? message = await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => EditRecoveryScreen(voucher: item),
                          ),
                        );
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                _infoChip("Total", item.amount),
                                _infoChip("Received", item.status),
                                _infoChip("Balance", item.bankBranch),
                                _infoChip("Bill Days", item.recoveryDate),
                                _infoChip("Over Days", item.recoveryDate),
...
//           crossAxisAlignment: CrossAxisAlignment.start,
//           children: [
//             _infoRow("Recovery Id", widget.voucher.rvNo ?? "N/A"),
//            // _infoRow("Recovery Date", formatDate(widget.voucher.createdAt ?? "")),
//             const SizedBox(height: 10),
//             _infoRow("Invoice No.", widget.invoice.invoiceNo ?? ""),
//             _infoRow("Invoice Date", formatDate(widget.invoice.invoiceDate ?? "")),
//             _infoRow("Customer", widget.voucher.customerName ?? ""),
//             _infoRow("Salesman", widget.voucher.salesmanName ?? ""),
...
class EditRecoveryScreen extends StatefulWidget {
  final RecoveryVoucher voucher;
  const EditRecoveryScreen({super.key, required this.voucher});
